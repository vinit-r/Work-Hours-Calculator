...
export function useWorkCalculator() {
  ...
  const updateManualBreak = (id, minutes) => {
    ...
    setState((prev) => ({
      ...prev,
      manualBreaks: prev.manualBreaks.map((b) =>
        b.id === id ? { ...b, minutes: m } : b
      ),
    }));
  };
  ...
  const calculations = useMemo(() => {
    const effectiveTotalMinutes =
      state.effectiveHours * 60 + state.effectiveMinutes;

    const totalManualBreakMinutes = state.manualBreaks.reduce(
    ...
    const totalSessionBreakMinutes = sessionBreaks.reduce(
    ...
    const entryAt = state.entryTime
      ? addMinutesToToday(state.entryTime, 0)
      : null;
    const leaveAt = entryAt
      ? entryAt.add(totalPresenceMinutes, 'minute')
      : null;
    ...
    const progressRatio =
      totalPresenceMinutes > 0
        ? Math.min(elapsedPresenceMinutes / totalPresenceMinutes, 1)
        : 0;

    const workedEffectiveMinutes = Math.round(
      progressRatio * effectiveTotalMinutes
    );

    const workedLabel = `${formatMinutesHM(
      workedEffectiveMinutes
    )} of ${formatMinutesHM(effectiveTotalMinutes)}`;

    const remainingLabel = formatCountdown(remainingMs);

    const leaveTimeLabel = formatTime12(
      state.entryTime && leaveAt.format('HH:mm')
    );
